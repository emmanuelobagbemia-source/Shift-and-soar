...
// Vercel automatically turns any file in /api into a live endpoint at /api/register.
//
// This file sends two emails through Resend whenever the booking form on the
// website is submitted:
//   1. An admin notification, with every field the client filled in.
//   2. A confirmation email back to the client.
//
// Both email templates live in this same file (see the two clearly marked
// sections below) so wording can be edited without touching anything else.
...
/* ============================================================================
   BRAND / SENDING SETTINGS
   Edit these two lines to change who receives the admin copy, and which
   address the emails are sent from. FROM_ADDRESS must use a domain that has
   been verified inside the Resend dashboard, or sending will fail.
============================================================================ */
const ADMIN_RECIPIENT = process.env.ADMIN_EMAIL;                 // <-- admin recipient is read from here
const FROM_ADDRESS = 'Shift and Soar <hello@shiftandsoar.co>';   // <-- sender address for both emails

// Brand colours, pulled from the site's own palette, used in the email HTML below.
const COLOR_NAVY = '#1B2A4A';
const COLOR_NAVY_DEEP = '#101a30';
...
/* ============================================================================
   SHARED EMAIL SHELL
   The header and footer wrapper used by both emails, so the two templates
   below only need to supply their own middle content. Editing the look of
   the header bar or footer here changes it for both emails at once.
============================================================================ */
...
/* ============================================================================
   ...
   Sent to ADMIN_RECIPIENT with everything the client entered in the form.
   Edit the subject line and the intro copy right here.
============================================================================ */
function adminEmailSubject(name) {
  return `New enquiry: ${name}`;                                 // <-- edit admin email subject here
}

function adminEmailHtml({ name, email, organisation, support, preferred_contact }) {
  // Each row only renders if the client actually filled it in.
  ...
  // ---- EDIT ADMIN EMAIL BODY TEXT BELOW ----
  const bodyHtml = `
...
/* ============================================================================
   ...
   Sent to the email address the client typed into the form.
   Edit the subject line and the thank you copy right here.
============================================================================ */
function clientEmailSubject() {
  return 'Thanks for reaching out to Shift and Soar';            // <-- edit client email subject here
}

function clientEmailHtml({ name, organisation, support, preferred_contact }) {
  // A short, warm recap of what they shared, shown only for fields they filled in.
  ...
  // ---- EDIT CLIENT EMAIL BODY TEXT BELOW ----
  const bodyHtml = `
...
module.exports = async (req, res) => {
  ...
  // Basic shape check, not a full RFC validator, just enough to catch obvious junk.
  ...
  try {
    const adminEmail = await resend.emails.send({
      from: FROM_ADDRESS,
      to: ADMIN_RECIPIENT,                                        // <-- admin recipient is used here
      subject: adminEmailSubject(name),
    ...
    if (adminEmail.error || clientEmail.error) {
      console.error('Resend error:', adminEmail.error, clientEmail.error);
      return res.status(502).json({ error: 'One or more emails failed to send' });
    }
